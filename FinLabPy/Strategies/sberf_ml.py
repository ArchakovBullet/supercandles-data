"""
SberF_ML v1.0 — оптимальные параметры из байесовской оптимизации
Стоп 3.8% | Безубыток 2.0% | FutOI 99M | ML 69%
"""
...
class SberF_ML(bt.Strategy):
    """
    SberF_ML — оптимизированная стратегия для фьючерса Сбербанка
    
    Параметры (байесовская оптимизация):
        Стоп: 3.8%
        Безубыток: 2.0%
        Порог FutOI: 99M
        Мин. уверенность ML: 69%
    """
    
    params = (
        ('stop_loss', 0.038),          # 3.8%
        ('breakeven_trigger', 0.02),   # 2.0%
        ('futoi_threshold', 99),       # 99M
        ('min_probability', 0.69),     # 69%
    )
    ...
    def _get_signal(self):
        ...
        buy_ok = (self.futoi.phys_net[0] > self.p.futoi_threshold and
                  self.fi.fi_ema13[0] > 0 and c > self.sma[0] and 30 < self.rsi[0] < 70)
    ...
    def next(self):
        ...
        if not self.ml_ready and bar_num >= 300:
            ...
            self.ml_ready = True
            if acc: print(f"   🤖 ML готов! Точность: {acc:.1%}")
        ...
        if self.entry_price and self.position:
            ...
            if hit:
                self._close('СТОП' if self.breakeven_moved else 'СТОП(-3.8%)', current_time)
                return
        ...
        print(f"{emoji} {signal} {current_time.strftime('%Y-%m-%d')} @ {entry:.1f} | "
              f"SL={self.stop_price:.1f} | ML:{proba:.0%}")
    ...
    def stop(self):
        ...
        print(f"\n{'='*80}")
        print(f"📊 SberF_ML — РЕЗУЛЬТАТЫ")
        print(f"{'='*80}")
        print(f"   Сигналов: {self.signal_count} | Пропущено ML: {self.ml_skip_count} | Сделок: {len(self.trades_log)}")
...
if __name__ == '__main__':
    ...
    cerebro.addstrategy(SberF_ML)
    cerebro.broker.set_cash(100000.0)
    cerebro.broker.setcommission(commission=0.0005)
    
    print(f"\n🚀 SberF_ML v1.0 | {TICKER} | {TIMEFRAME}")
    print(f"   Стоп: 3.8% | Безубыток: 2% | FutOI: 99M | ML: 69%")
    print("=" * 60)
