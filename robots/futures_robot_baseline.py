...
DATA_ROOT = ROOT / 'data'
DB_PATH = ROOT / 'robots' / 'futures_robot_baseline.db'
...
MAX_POSITIONS = 30  # BASELINE: поднято с 15 (25.09.2026) для полного теста
...
def open_position(ticker, direction, volume, score, price, atr):
    ...
    message = f"🤖 [BASELINE] ФЬЮЧЕРС-РОБОТ: {emoji} {direction} {ticker}: скор={score:.1f}, цена={price:.2f}"
...
def close_position(position_id, ticker, direction, exit_score, exit_price, reason):
    ...
    emoji = '🟢' if pnl > 0 else '🔴'
    message = f"🤖 [BASELINE] ФЬЮЧЕРС-РОБОТ: ЗАКРЫТИЕ {ticker}: PnL={pnl:+.2f}₽ ({reason}) {emoji}"
    send_vk_message(message)
            ...
            if ticker in open_tickers:
                ...
                if _is_trading_time:
                    ...
                    try:
                        ...
                        if 20 <= _perekos <= 60:
                            print(f'  📊 [BASELINE] {ticker}: перекос {_perekos:+.1f} — НЕ skip')
                    except Exception:
