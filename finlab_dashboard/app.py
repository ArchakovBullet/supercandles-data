...
def calculate_signals(df, df_d1=None, df_ts=None, atr_info=None, hi2_info=None):
    ...
    if signal_type == "LONG":
        lines.append(f"Высокая вероятность продолжения роста. Ближайшая цель — уровень сопротивления ({high_20:.2f}).")
    elif signal_type == "SHORT":
        lines.append(f"Высокая вероятность продолжения падения. Ближайшая цель — уровень поддержки ({low_20:.2f}).")
    elif signal_type == "BLOCKED_LONG":
        lines.append(f"Высокая вероятность продолжения падения или консолидации. Дождитесь разворота тренда или возврата цены выше POC ({poc_price:.2f}).")
    elif signal_type == "BLOCKED_SHORT":
        lines.append(f"Высокая вероятность продолжения роста или консолидации. Дождитесь разворота тренда или возврата цены ниже POC ({poc_price:.2f}).")
    elif signal_type == "WAIT_FOR_RETRACEMENT":
        lines.append(f"Ожидание коррекции вниз. После отката — вход в лонг от уровня поддержки ({low_20:.2f}).")
    elif signal_type == "WAIT_FOR_BOUNCE":
        lines.append(f"Ожидание отскока вверх. После отскока — вход в шорт от уровня сопротивления ({high_20:.2f}).")
    else:
    ...
    if signal_type == "LONG":
        lines.append(f"Входить в лонг от уровня поддержки ({low_20:.2f}). Стоп под минимум дня. Цель — сопротивление ({high_20:.2f}).")
    elif signal_type == "SHORT":
        lines.append(f"Входить в шорт от уровня сопротивления ({high_20:.2f}). Стоп над максимум дня. Цель — поддержка ({low_20:.2f}).")
    elif signal_type == "WAIT_FOR_RETRACEMENT":
        lines.append(f"Ждать отката к уровню поддержки ({low_20:.2f}). Входить в лонг только после подтверждения разворота.")
    elif signal_type == "WAIT_FOR_BOUNCE":
        lines.append(f"Ждать отскока к уровню сопротивления ({high_20:.2f}). Входить в шорт только после подтверждения разворота.")
    elif signal_type == "BLOCKED_LONG":
        lines.append(f"Не входить в лонг. Ждать: 1) возврата цены выше POC ({poc_price:.2f}), 2) разворота тренда на восходящий, 3) снижения перекупленности ниже 80%.")
    elif signal_type == "BLOCKED_SHORT":
        lines.append(f"Не входить в шорт. Ждать: 1) возврата цены ниже POC ({poc_price:.2f}), 2) разворота тренда на нисходящий, 3) снижения перепроданности выше 20%.")
    else:
